 ...
 */
public final class MessageMultiQueue {
    ...
    public MessageMultiQueue(int capacity, int numPriorities) {
    	...
    	queue = new MessageQueue[numPriorities];
    	
    	// initialize elements to queues for each priority
    	for (int i = 0; i < numPriorities; i++) {
    		queue[i] = new MessageQueue(capacity);
    	}
    }
    ...
    public MessageMultiQueue(MessageMultiQueue other) {
    	queue = new MessageQueue[other.priorities()];
    	
    	// assign copies of each queue to new array
    	for (int i = 0; i < priorities(); i++) {
    		queue[i] = new MessageQueue(other.queue[i]);
    	}
    }
    ...
    public boolean isFull() {
    	// full if enqueue for some priority could fail,
     ...
     */
    public int capacity() {
    	// sum of capacities -- is this right thing to do?
    	int cap = 0;
    	for (MessageQueue mq : queue) {
    		cap += mq.capacity();
    	}
