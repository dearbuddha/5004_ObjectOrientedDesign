...
public class ArrayDequeTest {
    ...
    public void testDeque() {
        ...
        // Test iterator
        Iterator<Character> dequeItr = deque.iterator();
        assertFalse("hasNext.0", dequeItr.hasNext());
        try {
        	dequeItr.next();
        	fail();
        } catch (NoSuchElementException e) {
        ...
        // test clear
        assertTrue("offer.0", deque.offerFirst(val[0]));
        assertTrue("offer.1", deque.offerFirst(val[1]));
        assertEquals("size.2", 2, deque.size());
        deque.clear();
        assertEquals("size.0", 0, deque.size());
        ...
        try {
        	// queue has 'h' first element
        	assertEquals("getFirst", val[0], deque.getFirst());
        } catch (NoSuchElementException e) {
        	fail();	// should succeed 
        }
        ...
        // Test iterator
        dequeItr = deque.iterator();
        assertTrue("hasNext.0", dequeItr.hasNext());
        try {
        	assertEquals(val[5], dequeItr.next());
        } catch (NoSuchElementException e) {
        	fail();
        }

        assertTrue("hasNext.1", dequeItr.hasNext());
        try {
        	assertEquals(val[2], dequeItr.next());
        } catch (NoSuchElementException e) {
        	fail();
        }

        assertTrue("hasNext.2", dequeItr.hasNext());
        try {
        	assertEquals(val[1], dequeItr.next());
        } catch (NoSuchElementException e) {
        	fail();
        }

        assertTrue("hasNext.3", dequeItr.hasNext());
        try {
        	assertEquals(val[0], dequeItr.next());
        } catch (NoSuchElementException e) {
        	fail();
        }

        assertTrue("hasNext.4", dequeItr.hasNext());
        try {
        	assertEquals(val[3], dequeItr.next());
        } catch (NoSuchElementException e) {
        	fail();
        }

        assertTrue("hasNext.5", dequeItr.hasNext());
        try {
        	assertEquals(val[4], dequeItr.next());
        } catch (NoSuchElementException e) {
        	fail();
        }

        // one past end
        assertFalse("hasNext.6", dequeItr.hasNext());
        try {
        	dequeItr.next();
        	fail();
        } catch (NoSuchElementException e) {
