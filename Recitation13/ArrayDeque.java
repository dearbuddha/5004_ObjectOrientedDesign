 ...
 */
final public class ArrayDeque<T> extends AbstractCollection<T> implements Deque<T> {

	/** Iterator for an ArrayDeque */
	private class ArrayDequeIterator implements Iterator<T> {
		/** Current index in ArrayDeque */
		int curIndex = 0;
		 ...
		 * @throws NoSuchElementException if next element not available 
		 */
		@Override
		public T next() {
			T nextT = get(curIndex);
			curIndex++;
			return nextT;
		}
	}
    ...
    /**
     * Returns an iterator to this instance
     * @return iterator to this instance
     */
     ...
     * 
     * @param c another collection of type T
     * @return true if added all
     */
    public boolean addAll(Collection<? extends T> c) {
    	Iterator<? extends T> citr = c.iterator();
    	while (citr.hasNext()) {
    		if (!add(citr.next())) {
    			return false;
    		}
    ...
    */
   public void clear() {
//	   // 1. new array of initial capacity
//       dequeArr = (T[]) new Object[4];  // should record initial capacity
//	   
//	   // 2. new array of current capacity
//       dequeArr = (T[]) new Object[dequeArr.length];
	   
	   // 3. Iterate array and set all elements to null
       // 3a whole array
	   java.util.Arrays.fill(dequeArr, null);
	   
//	   // 3b just elements occupied
//	   // arrayDequeIndex converts logical position into array index
//	   for (int i = size()-1; i >= 0; i--) dequeArr[arrayDequeIndex(i)] = null;
//	   
//	   // 4. Iterate and remove/poll elements
//	   while (!isEmpty()) pollFirst();
//	   

	   // must also free elements (not for #4)
	   front = rear = numOfElements = 0;
   }
